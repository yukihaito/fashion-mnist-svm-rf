...
if __name__ == "__main__":
    classifier_type = 'RF'#可通过classifier_type的更改选择SVM或随机森林（相比之下SVM真的慢）

    print('载入训练数据...')
    t = time()
    data = pd.read_csv('D:\\fminist\\fashion-mnist_train.csv', header=0, dtype=np.int)
    ...
    print('载入测试数据...')
    t = time()
    data_test = pd.read_csv('D:\\fminist\\fashion-mnist_test.csv', header=0, dtype=np.int)
    print('载入完成，耗时%f秒' % (time() - t))
...
#    plt.suptitle('分类器%s' % classifier_type, fontsize=15)
